...
path <- "/Users/Niha/Data-Analysis-BrainCheck/Niha_Analysis/cleaned_data/normal/03_02_17/normal_pop_2017_03_02.csv"
normal_pop <- read.csv(path, stringsAsFactors = FALSE)

batteries <- tbl(src_postgres("researchdb"),sql("SELECT * FROM batteries WHERE created_at >= '2017-03-02'"),check.names = FALSE)
...
# research_dat$date_of_birth <- as.character(research_dat$date_of_birth)
# research_dat$administered_at <-as.character(research_dat$administered_at)
# research_dat$completed_at <-as.character(research_dat$completed_at)
# research_dat$updated_at <-as.character(research_dat$updated_at)
# research_dat$notes_updated_at <- as.character(research_dat$notes_updated_at)
# research_dat$notes_created_at <- as.character(research_dat$created_at)
str(research_dat)
###write new dat to csv
write.csv(research_dat, file = "new_data_03_14_17.csv"  )
path2 <- "/Users/Niha/Data-Analysis-BrainCheck/Niha_Analysis/cleaned_data/new_data_03_14_17.csv"
...
write.csv(updated, file = "03_14_17.csv")
