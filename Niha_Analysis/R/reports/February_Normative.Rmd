...
a + facet_grid(agecat ~ device) +  labs(title="Stroop Effect") +
  ...
  geom_text(data = stroop_n, aes(x = 4.5, y= .8, label=frequency),size = 3,colour="black", inherit.aes=FALSE, parse=FALSE)
...
b + facet_grid(agecat ~ device) +  labs(title="Immediate Recall ") +
  ...
  geom_vline(data = immediate_recall, aes(xintercept= mean(immediate_recall$immediate_recall_correct, na.rm = T)),linetype = "dashed", color = "pink", size = 1) +
...
c + facet_grid(agecat ~ device) +  labs(title="Delayed Recall") +
 ...
 geom_text(data = delayed_recall_n, aes(x = 11, y= .8, label=frequency),size = 3,colour="black", inherit.aes=FALSE, parse=FALSE)
...
d + facet_grid(agecat ~ device) +  labs(title="Balance Test") +
  ...
  geom_text(data = balance_n, aes(x = 311, y= .8, label=frequency),size = 3,colour="black", inherit.aes=FALSE, parse=FALSE)
